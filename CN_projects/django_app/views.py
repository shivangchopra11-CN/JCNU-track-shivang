...
def get_projects(request, user_id):
    ...
    project_users = ProjectUser.objects.filter(user=user_object, is_mentor=True).values_list('project__name', flat=True)
    projects = []
...
def get_mentees(request, user_id):
    ...
    # print(request.body)
    user_object = User.objects.get(id=user_id)
    project_users = ProjectUser.objects.filter(user=user_object, is_mentor=True)
    projects = []
    for pu in project_users:
        projects.append(pu.project.name)
    users = []
    for proj_usr in project_users:
        cur_usrs = ProjectUser.objects.filter(project_id=proj_usr.project.id, is_mentor=False).values_list('user__name', flat=True)
        for usr in cur_usrs:
            users.append(usr)
...
def get_user_mentor(request, proj_id):
    ...
    proj_users = ProjectUser.objects.filter(project=proj_object, is_mentor=False).values_list('user__name', flat=True)
    project_mentor = ProjectUser.objects.get(project=proj_object, is_mentor=True).user.name
    users = []
